...
random_state = 42
batch_size = 32
model_weights_path = "../weights/UnetAutoencoder_gaussian_best.pth"
...
# --- Get input_shape for model (from STFT on one signal) ---
with torch.no_grad():
    test_example = X[0:1]
    padded = torch.nn.functional.pad(test_example, (pad, pad), mode="reflect")
    s = padded.squeeze(0).squeeze(0).numpy()
    _, _, Zxx = stft(s, fs=fs, nperseg=nperseg, noverlap=noverlap, boundary=None)
...
test_X = test_X.unsqueeze(0)  # [1, 1, T]
test_y = test_y.squeeze().numpy()

# --- Denoising (STFT -> U-Net -> ISTFT) ---
with torch.no_grad():
    ...
    out_mag = model(mag_tensor).squeeze(0).squeeze(0).numpy()
...
plt.ylim([-2,2])
plt.xlim([0,0.8])
plt.title(f"Sample #{sample_index} from test set ({dataset_type}) [U-Net AE]")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
